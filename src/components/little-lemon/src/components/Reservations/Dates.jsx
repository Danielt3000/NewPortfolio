...
function Dates({ days, childValue }) {
  ...
  const False = "bg-primary text-xl w-32 rounded-md text-white font-bold";
  const True =
    " animate-pulse bg-primary text-xl w-32 rounded-md text-white font-bold";
    ...
    <form className="grid grid-cols-2 place-items-center gap-3 ">
      {time[days].map((timeValue, index) => (
        <div
          ...
          key={index}
          className={selectedTime === timeValue ? True : False}
        >
